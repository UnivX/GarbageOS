...
volatile void* create_mp_init_routine_in_RAM(const void* paging_structure, MPInitializationData* init_data){
	...
	volatile void* raw_paging_structure = get_raw_paging_structure((void*)paging_structure);

	//copy cr3 page
	for(uint64_t i = 0; i < PAGE_SIZE; i++)
		((uint8_t*)cr3_copy)[i] = ((uint8_t*)raw_paging_structure)[i];
